      ...
      <div className="mt-6 rounded-xl border border-slate-200 bg-white p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="relative">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search assignment, order, customer..." className={`${inputCls} pl-9`} />
          </div>
          ...
          <table className="w-full min-w-[900px] text-left text-[13px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500">
                ...
                <th className="px-4 py-3 font-semibold">Assigned To / Team</th>
            ...
            <tbody>
              {filtered.map((a) => (
                <tr key={a.id} onClick={() => navigate(`/assignments/${a.id}`)} className="cursor-pointer border-b border-slate-100 transition-colors last:border-0 hover:bg-slate-50/60">
                  <td className="whitespace-nowrap px-4 py-3 font-semibold text-emerald-700 hover:underline">{a.id}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-slate-600">{a.orderId}</td>
                  <td className="max-w-[180px] truncate px-4 py-3 text-slate-600">{a.customer}</td>
                  <td className="max-w-[180px] truncate px-4 py-3 text-slate-700">{a.serviceName}</td>
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-slate-600">{fmtDate(a.assignedOn)}</td>
                  <td className="whitespace-nowrap px-4 py-3 text-slate-600">{a.expectedDelivery ? fmtDate(a.expectedDelivery) : '—'}</td>
                  <td className="whitespace-nowrap px-4 py-3"><Badge status={a.status} /></td>
