...
function Cashbook() {
  ...
  const downloadPdf = async () => {
    ...
    const cols = [
      ...
      { label: 'Cash In', w: 110, align: 'right' },
      { label: 'Cash Out', w: 110, align: 'right' },
    ];
    ...
    doc.setFontSize(16);
    doc.setTextColor(15, 23, 42);
    doc.text('CASH BOOK REPORT', rightX, 32, { align: 'right' });
      ...
      colMeta.forEach((c) => {
        if (c.label === 'Particulars') {
          ...
          if (c.label === 'Payment ID') {
            ...
            val = fmtDate(e.date);
            color = [71, 85, 105];
          } else if (c.label === 'Cash In') {
            val = fmtPdfAmt(e.income);
            if (e.income) {
              color = [4, 120, 87];
              fontStyle = 'bold';
            }
          } else if (c.label === 'Cash Out') {
    ...
    if (filtered.length === 0) {
      ...
      doc.text('No cash entries found matching current filter criteria.', M + totalTableW / 2, y + 22, { align: 'center' });
      y += 35;
    }
    ...
    if (filtered.length > 0) {
      ...
      const incomeCol = colMeta.find((c) => c.label === 'Cash In');
      if (incomeCol) {
        doc.setTextColor(4, 120, 87);
        doc.text(fmtPdfTotal(filteredIncome), incomeCol.rightX, y + 15.5, { align: 'right' });
      }

      const expenseCol = colMeta.find((c) => c.label === 'Cash Out');
      if (expenseCol) {
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      doc.setFontSize(7.5);
      doc.setTextColor(148, 163, 184);
      doc.text('Account Soft — Confidential Cashbook Report', M, H - 12);
      doc.text(`Page ${p} of ${totalPages}`, rightX, H - 12, { align: 'right' });
    }

    const filename = `Cashbook_Report_${dateFrom || 'all'}_to_${dateTo || 'all'}_${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(filename);
  };
