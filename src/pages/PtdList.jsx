          ...
          <table className="w-full min-w-[760px] text-left text-[13px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500">
                ...
                <th className="px-3.5 py-2.5 font-semibold">Allocated Hrs</th>
                <th className="px-3.5 py-2.5 font-semibold">Billing</th>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} onClick={() => navigate(`/ptd/${p.id}`)} className="cursor-pointer border-b border-slate-100 transition-colors last:border-0 hover:bg-slate-50/60">
                  <td className="whitespace-nowrap px-3.5 py-2.5 font-semibold text-emerald-700 hover:underline">{p.id}</td>
                  <td className="whitespace-nowrap px-3.5 py-2.5 text-slate-600">{p.orderId}</td>
                  <td className="max-w-[200px] truncate px-3.5 py-2.5 text-slate-800">{p.customer}</td>
                  <td className="whitespace-nowrap px-3.5 py-2.5 text-slate-700">{p.serviceName}</td>
                  <td className="whitespace-nowrap px-3.5 py-2.5"><Badge status={p.status} /></td>
                  <td className="whitespace-nowrap px-3.5 py-2.5 text-[12px] text-slate-600">{allocatedFor(p) > 0 ? <span className="font-medium text-slate-700">{allocatedFor(p)} hrs</span> : <span className="text-slate-300">—</span>}</td>
                  ...
                  <td className="whitespace-nowrap px-3.5 py-2.5 text-slate-600">{fmtDate(p.updatedAt)}</td>
                  <td className="whitespace-nowrap px-3.5 py-2.5 text-right">
                    ...
                    <button
                      ...
                      className="ml-1.5 inline-flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition-colors hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-600"
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan="9" className="px-4 py-12 text-center text-sm text-slate-400">No PTDs found.</td>
                </tr>
              )}
            </tbody>
          </table>
