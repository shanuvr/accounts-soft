...
function BankBook() {
  ...
  const downloadPdf = async () => {
    ...
    const cols = [
      { label: 'Payment ID', w: 100, align: 'left' },
      { label: 'Date', w: 90, align: 'center' },
      { label: 'Particulars', w: 360, align: 'left' },
      { label: 'Bank In', w: 110, align: 'right' },
      { label: 'Bank Out', w: 110, align: 'right' },
    ];
    ...
    // 3. Document Title & Period Info
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.setTextColor(15, 23, 42);
    doc.text('BANK BOOK REPORT', rightX, 30, { align: 'right' });
    ...
    filtered.forEach((e, idx) => {
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7.2);
      const subLines = e.particularsDetail ? doc.splitTextToSize(String(e.particularsDetail), (partCol?.w || 360) - 8) : [];
      ...
      colMeta.forEach((c) => {
        if (c.label === 'Particulars') {
          ...
          if (c.label === 'Payment ID') {
            ...
            val = fmtDate(e.date);
            color = [71, 85, 105];
          } else if (c.label === 'Bank In') {
            val = fmtPdfAmt(e.income);
            if (e.income) {
              color = [4, 120, 87];
              fontStyle = 'bold';
            }
          } else if (c.label === 'Bank Out') {
    ...
    if (filtered.length === 0) {
      ...
      doc.text('No bank entries found matching current filter criteria.', M + totalTableW / 2, y + 22, { align: 'center' });
      y += 35;
    }
    ...
    if (filtered.length > 0) {
      ...
      const incomeCol = colMeta.find((c) => c.label === 'Bank In');
      if (incomeCol) {
        doc.setTextColor(4, 120, 87);
        doc.text(fmtPdfTotal(filteredIncome), incomeCol.rightX, y + 15.5, { align: 'right' });
      }

      const expenseCol = colMeta.find((c) => c.label === 'Bank Out');
      if (expenseCol) {
    ...
    for (let p = 1; p <= totalPages; p++) {
      ...
      doc.setFontSize(7.5);
      doc.setTextColor(148, 163, 184);
      doc.text('Account Soft — Confidential Bankbook Report', M, H - 12);
      doc.text(`Page ${p} of ${totalPages}`, rightX, H - 12, { align: 'right' });
    }

    const filename = `BankBook_Report_${dateFrom || 'all'}_to_${dateTo || 'all'}_${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(filename);
  };
            ...
            <tbody>
              {filtered.map((e) => (
                <tr key={e.id} className="border-b border-slate-100 transition-colors last:border-0 hover:bg-slate-50/60">
                  ...
                  <td className="max-w-[400px] px-4 py-3 text-slate-800">
